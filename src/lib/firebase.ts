...
export const createNewAdminUser = async ({ email }: { email: string }): Promise<{ success: boolean; error?: string }> => {
    try {
        // Generate a secure random password for backend creation. User will not use this.
        const password = crypto.randomBytes(12).toString('base64').slice(0, 16);

        // Create the user in Firebase Auth.
        // This user will then be able to sign in using the standard passwordless flow.
        await firebaseCreateUser(auth, email, password);

        // We no longer send a custom email from here.
        // The admin can now instruct the new user to use the login page,
        // which will trigger the standard Firebase passwordless email.
...
export const uploadNewsletter = async (file: File, date: Date): Promise<void> => {
    ...
    try {
        ...
        // This is the expected error when the file does not exist.
        if (error.code === 'storage/object-not-found') {
            // The file doesn't exist, so we can proceed with the upload.
            try {
